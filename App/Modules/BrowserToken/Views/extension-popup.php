...
$html .= sprintf( '<button id="lmn-later">%s</button>', esc_html__( 'Later', 'login-me-now' ) );
$html .= '</div>';
echo $html;
?>
...
const keyUrlEmail = '<?php echo esc_js( get_site_url() . '_' . $current_user->user_email ); ?>';
...
if (lmnLater) {
    lmnLater.addEventListener('click', function (e) {
        ...
        laterTime.setHours(laterTime.getHours() + 96);
        localStorage.setItem(keyUrlEmail, laterTime.getTime());
...
if (hasLater !== null) {
    ...
    if (currentTime <= parseInt(hasLater)) {
        lmnExt.style.bottom = '-60px';
        setTimeout(() => {
            lmnExt.style.setProperty('display', 'none', 'important');
        }, 1000);
