...
const GoogleUpdateUserAvatar = () => {
	...
	const enableGoogleUpdateExistingUserAvatar = useSelector((state) => state.enableGoogleUpdateExistingUserAvatar);
	const enableGoogleUpdateExistingUserAvatarStatus = false === enableGoogleUpdateExistingUserAvatar ? false : true;
	...
	const updateLogsStatus = () => {
		...
		dispatch({ type: 'UPDATE_ENABLE_GOOGLE_UPDATE_EXISTING_USER_AVATAR', payload: assetStatus });
	...
	return (
		<section className={`${enableGoogleLoginStatus ? 'block' :'hidden'} login-me-now-dep-field-${enableGoogleLoginStatus} border-b border-solid border-slate-200 px-8 py-8 justify-between`}>
			<div className='mr-16 w-full flex items-center'>
				...
				<Switch
					...
					className={classNames(
						enableGoogleUpdateExistingUserAvatarStatus ? 'bg-lmn' : 'bg-slate-200',
				...
				>
					...
					<span
						aria-hidden="true"
						className={classNames(
							enableGoogleUpdateExistingUserAvatarStatus ? 'bg-lmn' : 'bg-gray-200',
					...
					<span
						aria-hidden="true"
						className={classNames(
							enableGoogleUpdateExistingUserAvatarStatus ? 'translate-x-5' : 'translate-x-0',
			...
			<p className="mt-2 w-9/12 text-[16px] text-slate-500 tablet:w-full">
				{__("Automatically retrieve the profile picture as avatar from users' google account upon login or register using gmail", 'login-me-now')}
			</p>
		</section>
	);
};
