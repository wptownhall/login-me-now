...
const FacebookInUpdateExistingUserData = () => {
  ...
  const enableFacebookInUpdateExistingUserData = useSelector(
    (state) => state.enableFacebookInUpdateExistingUserData
  );
  const enableFacebookInUpdateExistingUserDataStatus =
    false === enableFacebookInUpdateExistingUserData ? false : true;

  const updateLogsStatus = () => {
    let assetStatus;
    if (enableFacebookInUpdateExistingUserData === false) {
      assetStatus = true;
    } else {
      assetStatus = false;
    }
  ...
  return (
    <section
      className={`${
        enableFacebookLogin ? "block" : "hidden"
    ...
    >
      <div className="mr-16 w-full flex items-center">
        <h3 className="p-0 flex-1 justify-right inline-flex text-xl leading-6 font-semibold text-slate-800">
          {__("Update existing user data", "login-me-now")}
        </h3>
        <Switch
          checked={enableFacebookInUpdateExistingUserDataStatus}
          onChange={updateLogsStatus}
          className={classNames(
            enableFacebookInUpdateExistingUserDataStatus
              ? "bg-lmn"
              : "bg-slate-200",
      ...
      <p className="mt-2 w-9/12 text-sm text-slate-500 tablet:w-full">
        {__(
          "Update First, Last, Display & Nick Name according to Facebook profile.",
          "login-me-now"
        )}
      </p>
    </section>
