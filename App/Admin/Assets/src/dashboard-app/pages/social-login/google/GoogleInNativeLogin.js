...
const GoogleInNativeLogin = () => {
  ...
  const enableGoogleNativeLogin = useSelector(
    (state) => state.enableGoogleNativeLogin
  );
  const enableGoogleNativeLoginStatus =
    false === enableGoogleNativeLogin ? false : true;
  ...
  const updateNativeLoginStatus = () => {
    ...
    dispatch({
      type: "UPDATE_ENABLE_GOOGLE_NATIVE_LOGIN",
      payload: assetStatus,
    });
  ...
  return (
    ...
    >
      ...
      <p className="mt-6 w-9/12 text-[16px] text-slate-500 tablet:w-full leading-[1.7]">
        {__("Check, what would it look like!", "login-me-now")}
      </p>
    </section>
  );
};
